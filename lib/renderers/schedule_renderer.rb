class ScheduleRenderer
  ...
  def render
    ...
    (CONFIG['hour']['start']..CONFIG['hour']['end']).each do |hour|
      ...
      (monday..friday).each do |day|
        schedule_r << render_scheduling(@days.dig(day.to_time.to_i, hour))
      end
      schedule_r << '</tr>'
    end
  ...
  private
    def render_scheduling(scheduling)
      if scheduling
        SchedulingRenderer.new(scheduling, @user).render
      else
        EmptySchedulingRenderer.new.render
      end
    end
end
